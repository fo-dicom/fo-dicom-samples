...
const string _storagePath = @".\DICOM";
// values of the Query Retrieve Server to test with.
const string _qrServerHost =  "www.dicomserver.co.uk"; // "localhost"; //
const int _qrServerPort =  104; // 8001; //
const string _qrServerAET =  "STORESCP"; //"QRSCP";
const string _aet = "FODICOMSCU";
...
static DicomCFindRequest CreateStudyRequestByPatientName(string patientName)
{
...
static DicomCFindRequest CreateSeriesRequestByStudyUID(string studyInstanceUID)
{
    // there is a built in function to create a Study-level CFind request very easily: 
...
static DicomCGetRequest CreateCGetBySeriesUID(string studyUID, string seriesUID)
{
...
static DicomCMoveRequest CreateCMoveBySeriesUID(string destination, string studyUID, string seriesUID)
{
...
static DicomCMoveRequest CreateCMoveByStudyUID(string destination, string studyUID)
{
...
static void DebugStudyResponse(DicomCFindResponse response)
{
...
static void DebugSerieResponse(DicomCFindResponse response)
{
    try
    {
        if (response.Status == DicomStatus.Pending)
...
static void SaveImage(DicomDataset dataset)
{
